...
const slideDeckDefaultFilename = "slide-deck.json"

const systemPrompt = `You are a presentation draft assistant.
Generate slide deck drafts in JSON structure that includes:
1. title
2. audience
3. slides
4. speakerNotes

You MUST only output draft content.
You MUST NOT claim that a real pptx file was generated.
You MUST NOT write files or run python, libreoffice, or shell commands.

When possible, include JSON fenced blocks with:
` + "```json:slide-deck.json" + `
or:
` + "```json" + `
...
func processTaskWithLLM(llm *adk.LLMClient, ctx *adk.Context, messages []a2a.Message) error {
	...
	if err != nil {
		return fmt.Errorf("slide deck generation failed: %w", err)
	}

	for _, artifact := range extractSlideDeckArtifacts(fullResponse.String()) {
...
func extractSlideDeckArtifacts(text string) []adk.Artifact {
	blocks := parseJSONBlocks(text, slideDeckDefaultFilename)
