...
const versionSnapshotDefaultFilename = "version-snapshot.json"

const systemPrompt = `You are a version planning assistant.
Generate:
1. Version summary.
2. Parent/base version reference.
3. Related patch/artifact links.
4. Rollback plan notes.

You MUST NOT claim executing git tag, git commit, git checkout, git push, git reset, or any shell command.
You MUST NOT perform repository write operations.

When possible, include JSON fenced blocks with:
` + "```json:version-snapshot.json" + `
or:
` + "```json" + `
...
func processTaskWithLLM(llm *adk.LLMClient, ctx *adk.Context, messages []a2a.Message) error {
	...
	if err != nil {
		return fmt.Errorf("version snapshot generation failed: %w", err)
	}

	for _, artifact := range extractVersionArtifacts(fullResponse.String()) {
...
func extractVersionArtifacts(text string) []adk.Artifact {
	blocks := parseJSONBlocks(text, versionSnapshotDefaultFilename)
