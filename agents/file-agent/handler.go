...
const fileSummaryDefaultFilename = "file-summary.json"

const systemPrompt = `You are a file-content analysis assistant.
You can ONLY analyze file text provided by the caller in the message content.
You MUST NOT claim that you read local files, opened file paths, or used shell commands.
You MUST NOT unzip archives, OCR images, or access file systems.
...
func processTaskWithLLM(llm *adk.LLMClient, ctx *adk.Context, messages []a2a.Message) error {
	...
	if err != nil {
		return fmt.Errorf("file summary generation failed: %w", err)
	}

	for _, artifact := range extractFileSummaryArtifacts(fullResponse.String()) {
...
func extractFileSummaryArtifacts(text string) []adk.Artifact {
	blocks := parseJSONBlocks(text, fileSummaryDefaultFilename)
