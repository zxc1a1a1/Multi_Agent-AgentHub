...
const releaseReportDefaultFilename = "release-report.json"

const systemPrompt = `You are a release reporting assistant.
Generate:
1. release summary
2. change list
3. test status
4. risk notes
5. rollback suggestions

You MUST only generate reports from provided text.
You MUST NOT claim that a release is already published.
You MUST NOT execute or claim executing git tag, git commit, git push, gh release, npm publish, docker push, or shell commands.
...
func processTaskWithLLM(llm *adk.LLMClient, ctx *adk.Context, messages []a2a.Message) error {
	...
	if err != nil {
		return fmt.Errorf("release report generation failed: %w", err)
	}

	for _, artifact := range extractReleaseArtifacts(fullResponse.String()) {
...
func extractReleaseArtifacts(text string) []adk.Artifact {
	blocks := parseJSONBlocks(text, releaseReportDefaultFilename)
