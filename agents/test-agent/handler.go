...
const testReportDefaultFilename = "test-report.json"

const systemPrompt = `You are a test analysis assistant.
You can ONLY analyze logs provided in the user messages.
You MUST NOT claim that you executed go test, npm test, docker, shell commands, or any real tools.
Return:
1. Observed failures/success from provided logs.
...
func processTaskWithLLM(llm *adk.LLMClient, ctx *adk.Context, messages []a2a.Message) error {
	...
	for _, artifact := range extractTestArtifacts(fullResponse.String()) {
...
func extractTestArtifacts(text string) []adk.Artifact {
	blocks := parseJSONBlocks(text, testReportDefaultFilename)
