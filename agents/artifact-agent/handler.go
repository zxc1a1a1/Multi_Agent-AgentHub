...
const artifactManifestDefaultFilename = "artifact-manifest.json"

const systemPrompt = `You are an artifact normalization assistant.
Convert raw output descriptions into structured artifact manifest JSON.
You must not write object storage, local files, or databases.
...
func processTaskWithLLM(llm *adk.LLMClient, ctx *adk.Context, messages []a2a.Message) error {
	...
	if err != nil {
		return fmt.Errorf("artifact manifest generation failed: %w", err)
	}

	for _, artifact := range extractArtifactManifestArtifacts(fullResponse.String()) {
...
func extractArtifactManifestArtifacts(text string) []adk.Artifact {
	blocks := parseJSONBlocks(text, artifactManifestDefaultFilename)
