...
const reviewReportDefaultFilename = "review-report.json"

const systemPrompt = `You are a review assistant.
Review the provided content against requirements, quality, and safety criteria.
Return:
...
func processTaskWithLLM(llm *adk.LLMClient, ctx *adk.Context, messages []a2a.Message) error {
	...
	if err != nil {
		return fmt.Errorf("review report generation failed: %w", err)
	}

	for _, artifact := range extractReviewArtifacts(fullResponse.String()) {
...
func extractReviewArtifacts(text string) []adk.Artifact {
	blocks := parseJSONBlocks(text, reviewReportDefaultFilename)
