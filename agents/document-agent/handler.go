...
const systemPrompt = `You are a document writing assistant.
When the user asks for docs, reports, PRDs, or README files:
1. Provide a concise structure summary.
2. Output Markdown content in fenced blocks using one of:
   ` + "```markdown:report.md" + `
   ` + "```md:README.md" + `
   ` + "```markdown" + `
3. Use meaningful filenames when possible.
4. If you generate multiple documents, put each in its own block.
5. Do not output an unescaped standalone triple-backtick line inside block content.
6. Keep the document clear, practical, and ready for handoff.`
...
func processTaskWithLLM(llm *adk.LLMClient, ctx *adk.Context, messages []a2a.Message) error {
	...
	for _, artifact := range extractDocumentArtifacts(fullResponse.String()) {
...
func extractDocumentArtifacts(text string) []adk.Artifact {
