...
const webResearchDefaultFilename = "web-research.json"

const systemPrompt = `You are a web research summary assistant.
You can only analyze URL/title/body excerpts that the caller provided in text.
You MUST NOT claim that you actively visited pages.
You MUST NOT fetch URLs, browse the internet, access intranet addresses, or access file:// paths.
You MUST NOT execute shell commands or hidden tools.

When possible, include JSON fenced blocks with:
` + "```json:web-research.json" + `
or:
` + "```json" + `
...
func processTaskWithLLM(llm *adk.LLMClient, ctx *adk.Context, messages []a2a.Message) error {
	...
	if err != nil {
		return fmt.Errorf("web research summary generation failed: %w", err)
	}

	for _, artifact := range extractWebResearchArtifacts(fullResponse.String()) {
...
func extractWebResearchArtifacts(text string) []adk.Artifact {
	blocks := parseJSONBlocks(text, webResearchDefaultFilename)
