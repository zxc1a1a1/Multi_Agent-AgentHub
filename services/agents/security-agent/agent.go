...
const (
	defaultAgentName        = "security-agent"
	defaultAgentDescription = "Scan code for vulnerabilities, check dependencies, audit configs, and detect secrets."
...
func WithModel(m adk.Model) SecurityAgentOption {
	return func(a *SecurityAgent) {
		if a == nil { return }
		a.llm = m
	}
}

func NewSecurityAgent(cfg Config, opts ...SecurityAgentOption) *SecurityAgent {
	...
	a := &SecurityAgent{name: name, description: description, version: version}
...
func (a *SecurityAgent) Generate(ctx context.Context, req *adk.GenerateRequest) (*adk.GenerateResponse, error) {
