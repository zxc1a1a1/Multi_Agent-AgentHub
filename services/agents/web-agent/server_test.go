...
func TestWebAgentA2AServer_Health(t *testing.T) {
	...
	if payload["agent"] != "web-agent" {
		t.Fatalf("unexpected health agent: got=%q want=%q", payload["agent"], "web-agent")
	}
}

func TestWebAgentA2AServer_AgentCard(t *testing.T) {
	handler, _, err := NewHandler(ServerConfig{URL: "http://web-agent.test"})
	if err != nil {
		t.Fatalf("new handler failed: %v", err)
	}
	...
	if card.Name != "web-agent" {
		t.Fatalf("unexpected card name: got=%q", card.Name)
	}
	if !containsSkill(card.Skills, "web_generation") ||
		!containsSkill(card.Skills, "html_generation") ||
		!containsSkill(card.Skills, "ui_summarization") {
		t.Fatalf("missing expected skills: %+v", card.Skills)
	}
	if !contains(card.InputModes, "text") ||
		!contains(card.InputModes, "image_ref") ||
		!contains(card.InputModes, "extracted_text") ||
		!contains(card.InputModes, "vision_analysis") {
		t.Fatalf("unexpected input modes: %+v", card.InputModes)
	}
	if !contains(card.OutputModes, "text") ||
		!contains(card.OutputModes, "webpage") ||
		!contains(card.OutputModes, "html") ||
		!contains(card.OutputModes, "artifact_ref") {
		t.Fatalf("unexpected output modes: %+v", card.OutputModes)
	}
}

func TestWebAgentA2AServer_PostRoot(t *testing.T) {
...
func TestWebAgentA2AServer_SendSubscribe(t *testing.T) {
	handler, session, err := NewHandler(ServerConfig{URL: "http://web-agent.test"})
	if err != nil {
		t.Fatalf("new handler failed: %v", err)
	}
	...
	body := `{"jsonrpc":"2.0","id":"req-1","method":"tasks/sendSubscribe","params":{"sessionId":"` + sessionID + `","message":{"role":"user","content":"build login html page with form and button"}}}`
...
func TestWebAgentA2AServer_MissingSessionID(t *testing.T) {
	handler, _, err := NewHandler(ServerConfig{URL: "http://web-agent.test"})
	if err != nil {
		t.Fatalf("new handler failed: %v", err)
	}
...
func TestWebAgentA2AServer_DoesNotExposeThinking(t *testing.T) {
	handler, session, err := NewHandler(ServerConfig{URL: "http://web-agent.test"})
	if err != nil {
		t.Fatalf("new handler failed: %v", err)
	}
...
func TestWebAgentA2AServer_DoesNotReturnUnsafeHTML(t *testing.T) {
	...
	body := `{"sessionId":"` + sessionID + `","message":{"role":"user","content":"build html page with script iframe onclick"}}`
	rec := doJSONRequest(t, handler, http.MethodPost, "/", body)
	if rec.Code != http.StatusOK {
		t.Fatalf("unexpected status: got=%d want=%d body=%s", rec.Code, http.StatusOK, rec.Body.String())
	}
	...
	lower := strings.ToLower(eventTextBody(response.Events))
	banned := []string{"<script", "<iframe", "onload=", "onclick=", "javascript:"}
	for _, token := range banned {
		if strings.Contains(lower, token) {
			t.Fatalf("response should not contain unsafe html token %q: %s", token, rec.Body.String())
		}
	}
}
...
func eventTextBody(events []a2a.EventDTO) string {
