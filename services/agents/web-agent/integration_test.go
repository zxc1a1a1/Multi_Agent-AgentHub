...
func TestIntegration_WebAgent_ClientRoundTrip(t *testing.T) {
...
func TestIntegration_WebAgent_RemoteAgentRoundTrip(t *testing.T) {
	t.Setenv("OPENAI_API_KEY", "mock_env_key_should_not_be_used")

	httpServer, sessionID := setupIntegrationServer(t)
	defer httpServer.Close()

	remote := a2a.NewRemoteAgent("remote-web-agent", httpServer.URL, a2a.WithSessionID(sessionID))
	resp, err := remote.Generate(context.Background(), &adk.GenerateRequest{
		Contents: []*adk.Content{
			{
				Role: adk.RoleUser,
				Parts: []adk.Part{
					adk.TextPart{Text: "please build a login html page"},
				},
			},
		},
	})
	...
	if !strings.Contains(textPart.Text, "<section") {
		t.Fatalf("expected html output in response, got=%q", textPart.Text)
	}
...
func TestIntegration_WebAgent_AgentCard(t *testing.T) {
	httpServer, sessionID := setupIntegrationServer(t)
	defer httpServer.Close()
...
func TestIntegration_WebAgent_NoLegacyImport(t *testing.T) {
	...
	for _, dep := range info.Deps {
		...
		if strings.Contains(dep.Path, "/agents/web-agent") {
			t.Fatalf("legacy web-agent package should not be imported, found=%q", dep.Path)
		}
	}
}

func TestIntegration_WebAgent_DoesNotReturnUnsafeHTML(t *testing.T) {
	httpServer, sessionID := setupIntegrationServer(t)
	defer httpServer.Close()
	...
	resp, err := client.Send(context.Background(), httpServer.URL, a2a.RunRequest{
		...
		Message: a2a.Message{
			Role:    "user",
			Content: "build html with script iframe onclick",
		},
	})
	if err != nil {
		t.Fatalf("client round-trip failed: %v", err)
	...
	lower := strings.ToLower(eventTextBody(resp.Events))
	banned := []string{"<script", "<iframe", "onload=", "onclick=", "javascript:"}
	for _, token := range banned {
		if strings.Contains(lower, token) {
			t.Fatalf("response should not contain unsafe html token %q: %+v", token, resp.Events)
		}
	}
}
...
func setupIntegrationServer(t *testing.T) (*httptest.Server, string) {
	...
	handler, session, err := NewHandler(ServerConfig{URL: "http://web-agent.integration"})
	if err != nil {
