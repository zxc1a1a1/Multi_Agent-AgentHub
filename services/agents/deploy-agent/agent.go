...
const (
	defaultAgentName        = "deploy-agent"
	defaultAgentDescription = "Generate deployment plans, check environment health, and create rollback strategies."
...
type DeployAgentOption func(*DeployAgent)

func WithModel(m adk.Model) DeployAgentOption {
	return func(a *DeployAgent) {
		if a == nil { return }
		a.llm = m
	}
}

func NewDeployAgent(cfg Config, opts ...DeployAgentOption) *DeployAgent {
	...
	a := &DeployAgent{name: name, description: description, version: version}
...
func (a *DeployAgent) Name() string {
	if a == nil || strings.TrimSpace(a.name) == "" { return defaultAgentName }
	return a.name
}

func (a *DeployAgent) Generate(ctx context.Context, req *adk.GenerateRequest) (*adk.GenerateResponse, error) {
