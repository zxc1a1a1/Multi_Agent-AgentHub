...
const (
	defaultAgentName        = "context-agent"
	defaultAgentDescription = "Compress and manage conversation context, generate summaries, extract memories."
...
type ContextAgentOption func(*ContextAgent)

func WithModel(m adk.Model) ContextAgentOption {
	return func(a *ContextAgent) {
		if a == nil { return }
		a.llm = m
	}
}

func NewContextAgent(cfg Config, opts ...ContextAgentOption) *ContextAgent {
	...
	a := &ContextAgent{name: name, description: description, version: version}
...
func (a *ContextAgent) Name() string {
	if a == nil || strings.TrimSpace(a.name) == "" { return defaultAgentName }
	return a.name
}

func (a *ContextAgent) Generate(ctx context.Context, req *adk.GenerateRequest) (*adk.GenerateResponse, error) {
