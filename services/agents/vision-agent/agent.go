...
const (
	defaultAgentName        = "vision-agent"
	defaultAgentDescription = "Analyze images, extract text via OCR, and audit visual content."
...
type VisionAgentOption func(*VisionAgent)

func WithModel(m adk.Model) VisionAgentOption {
	return func(a *VisionAgent) {
		if a == nil { return }
		a.llm = m
	}
}

func NewVisionAgent(cfg Config, opts ...VisionAgentOption) *VisionAgent {
	...
	a := &VisionAgent{name: name, description: description, version: version}
...
func (a *VisionAgent) Name() string {
	if a == nil || strings.TrimSpace(a.name) == "" { return defaultAgentName }
	return a.name
}

func (a *VisionAgent) Generate(ctx context.Context, req *adk.GenerateRequest) (*adk.GenerateResponse, error) {
